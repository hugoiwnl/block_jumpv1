...
export default class Renderer {
    ...
    render(scene, camera) {
        ...
        scene.traverse(
            node => {
                if(node.translation[0]==0 && node.translation[1]==3.6){
                    this.skybox1=node;
                }
                if(node.translation[0]==0 && node.translation[1]==4.5){
                    this.skybox2=node;
                }
                if(node.translation[0]==0 && node.translation[1]==9 && node.translation[2]==-30){
                    this.skybox3=node;
                }
                matrixStack.push(mat4.clone(matrix));
                mat4.mul(matrix, matrix, node.transform);
                if (node.gl.vao) {
                    if(node==this.skybox1 || node==this.skybox2 || node==this.skybox3){
